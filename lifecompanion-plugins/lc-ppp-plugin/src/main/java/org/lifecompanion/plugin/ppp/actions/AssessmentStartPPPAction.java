...
public class AssessmentStartPPPAction extends SimpleUseActionImpl<UseActionTriggerComponentI> implements KeepEvaluatorActionI {
    private final BooleanProperty takePrevEvaluator;

    public AssessmentStartPPPAction() {
        super(UseActionTriggerComponentI.class);
        this.nameID = "ppp.plugin.actions.assessment.start.ppp.name";
        this.staticDescriptionID = "ppp.plugin.actions.assessment.start.ppp.description";
    ...
    public void execute(final UseActionEvent event, final Map<String, UseVariableI<?>> variables) {
        AssessmentService.INSTANCE.startPPPAssessment(this.takePrevEvaluatorProperty().get());
    }

    @Override
    public Element serialize(final IOContextI contextP) {
        Element node = super.serialize(contextP);
        XMLObjectSerializer.serializeInto(AssessmentStartPPPAction.class, this, node);
        return node;
    }
    ...
    public void deserialize(final Element nodeP, final IOContextI contextP) throws LCException {
        ...
        XMLObjectSerializer.deserializeInto(AssessmentStartPPPAction.class, this, nodeP);
    }
}
