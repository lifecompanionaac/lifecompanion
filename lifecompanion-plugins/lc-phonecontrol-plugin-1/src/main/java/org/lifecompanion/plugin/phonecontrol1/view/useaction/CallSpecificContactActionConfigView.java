...
import org.lifecompanion.plugin.phonecontrol1.action.CallSpecificContactAction;
...
public class CallSpecificContactActionConfigView extends VBox
    ...
    @Override
    public void editEnds(final CallSpecificContactAction action) {
        action.phoneNumberProperty().set(fieldPhoneNumber.getText());
        action.contactNameProperty().set(fieldContactName.getText());
    }

    @Override
    public Class<CallSpecificContactAction> getConfiguredActionType() {
        return CallSpecificContactAction.class;
    }
}
