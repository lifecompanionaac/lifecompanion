...
import org.lifecompanion.ui.configurationcomponent.editmode.categorizedelement.useevent.available.PeriodicConfigView;
import org.lifecompanion.util.javafx.FXControlUtils;
import org.lifecompanion.plugin.phonecontrol.ADBService;
        ...
        this.installAppButton.setOnAction(event -> {
            ...
            Thread installThread = new Thread(() -> {
                ...
                if (deviceSerialNumber != null) {
                    ...
                    Platform.runLater(() -> {
                        labelInstallResult.setText(Translation.getText(isInstalled ? "phonecontrol.plugin.config.label.install.app.success" : "phonecontrol.plugin.config.label.install.app.error"));
                    });
                } else {
                    Platform.runLater(() -> {
    ...
    private void refreshDeviceList() {
        ...
        if (deviceSerialNumber == null) { // If no device was selected
            ...
            if (selectedDevice != null) { // If the device was found, we select it
                ...
                selectDeviceComboBox.getItems().add(new Device(Translation.getText("phonecontrol.plugin.config.label.device.unfound"), deviceSerialNumber));
