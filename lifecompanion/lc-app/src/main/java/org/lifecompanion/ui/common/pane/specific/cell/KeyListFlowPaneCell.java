...
public class KeyListFlowPaneCell extends FlowPaneListView.FlowPaneListCell<KeyListNodeI> {
    ...
    public KeyListFlowPaneCell(FlowPaneListView<KeyListNodeI> flowPaneListView) {
        super(flowPaneListView);
    ...
    @Override
    protected void bindItem(KeyListNodeI item) {
        ...
        rectangleColors.strokeProperty().bind(Bindings.createObjectBinding(() -> selectedProperty().get() ? Color.BLUE : item.strokeColorProperty().get(),
                this.selectedProperty(), item.strokeColorProperty()));
        rectangleColors.fillProperty().bind(item.backgroundColorProperty());
        rectangleColors.visibleProperty().bind(selectedProperty().or(item.strokeColorProperty().isNotNull().or(item.backgroundColorProperty().isNotNull())));
        ...
        item.addExternalLoadingRequest(nodeIdForImageLoading);
    }

    @Override
    protected void unbindItem(KeyListNodeI item) {
        ...
        glyphPane.getChildren().clear();
        item.removeExternalLoadingRequest(nodeIdForImageLoading);
    }
}
