...
public class ConfigAddView extends BorderPane implements LCViewInitHelper, ProfileConfigStepViewI {
    ...
    public void initUI() {
        ...
        this.buttonAddFromDefault = ConfigUIUtils.createActionTableEntry(4, "config.selection.add.from.default.config.button",
                GlyphFontHelper.FONT_AWESOME.create(FontAwesome.Glyph.TH_LARGE).size(28).color(LCGraphicStyle.MAIN_DARK), gridPaneActions,
        ...
        this.buttonDuplicate = ConfigUIUtils.createActionTableEntry(6, "config.selection.create.duplicate.config.button",
                GlyphFontHelper.FONT_AWESOME.create(FontAwesome.Glyph.COPY).size(28).color(LCGraphicStyle.MAIN_DARK), gridPaneActions,
                () -> ConfigActionController.INSTANCE.executeAction(new LCConfigurationActions.DuplicateEditAction((buttonDuplicate))));
