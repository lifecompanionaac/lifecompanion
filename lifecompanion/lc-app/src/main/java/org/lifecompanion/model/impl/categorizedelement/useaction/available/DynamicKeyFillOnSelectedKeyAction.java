...
public class DynamicKeyFillOnSelectedKeyAction extends SimpleUseActionImpl<GridPartKeyComponentI> {
    @SuppressWarnings("FieldCanBeLocal")
    private final StringProperty targetKeyId;
    private final ComponentHolderById<GridPartKeyComponentI> targetKey;

    @SuppressWarnings("FieldCanBeLocal")
    public DynamicKeyFillOnSelectedKeyAction() {
        super(GridPartKeyComponentI.class);
        this.order = 40;
        this.category = DefaultUseActionSubCategories.DYNAMIC_KEYS;
        this.parameterizableAction = true;
        this.nameID = "use.action.fill.dynamic.key.on.selected.key.name";
        this.staticDescriptionID = "use.action.fill.dynamic.key.on.selected.key.description";
        this.configIconPath = "configuration/icon_fill_selected_key_fill.png";
    ...
    public void execute(final UseActionEvent eventP, final Map<String, UseVariableI<?>> variables) {
        ...
        if (targetKeyComp != null) {
            DynamicKeyFillController.INSTANCE.fillWith(targetKeyComp, this.parentComponentProperty().get());
        }
    }

    @Override
    public Element serialize(final IOContextI contextP) {
        return XMLObjectSerializer.serializeInto(DynamicKeyFillOnSelectedKeyAction.class, this, super.serialize(contextP));
    }

    @Override
    public void deserialize(final Element nodeP, final IOContextI contextP) throws LCException {
        super.deserialize(nodeP, contextP);
        XMLObjectSerializer.deserializeInto(DynamicKeyFillOnSelectedKeyAction.class, this, nodeP);
    }
}
