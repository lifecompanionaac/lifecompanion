...
public enum LifeCompanionControlServerEndpoint implements LifeCompanionControlServerEndpointI {
    ...
    INDICATION_TARGET_SHOW_LOCATION("indication/target/show/location",
            EndpointHttpMethod.POST,
            "Show the target indication to a specific location in the current main grid. If the target is reached, an specific use event is generated (Cible de déplacement atteinte). The behavior can be : HIDE_ON_REACHED (the target is hidden when reached on moving), HIDE_ON_ACTIVATION (the target will be hidden if the cursor is on the target and activated), KEEP (the target will always be showing until hide is called). Default is HIDE_ON_REACHED.",
            List.of(new ShowIndicationTargetDto(ColorUtils.toWebColorWithAlpha(Color.GREEN), 5.0, IndicationTargetBehavior.HIDE_ON_REACHED, 2, 4), new ShowIndicationTargetDto(null, null, null, 1, 3)),
            List.of(ActionConfirmationDto.ok())),
    INDICATION_TARGET_SHOW_RANDOM("indication/target/show/random",
            EndpointHttpMethod.POST,
            "Show the target indication to a random location in the current main grid. If the target is reached, an specific use event is generated (Cible de déplacement atteinte).  The behavior can be : HIDE_ON_REACHED (the target is hidden when reached on moving), HIDE_ON_ACTIVATION (the target will be hidden if the cursor is on the target and activated), KEEP (the target will always be showing until hide is called). Default is HIDE_ON_REACHED.",
            List.of(new ShowIndicationRandomTargetDto(ColorUtils.toWebColorWithAlpha(Color.GREEN), 5.0, IndicationTargetBehavior.HIDE_ON_ACTIVATION), new ShowIndicationRandomTargetDto(null, null, null)),
            List.of(ActionConfirmationDto.ok())),
