...
public class GridActions {
    ...
    public static class ReplaceMultiCompAction implements UndoRedoActionI {
        private final List<ReplaceComponentAction> replaceActions;
        private final GridPartComponentI toPaste;
        private final List<GridPartKeyComponentI> destKeys;
        ...
        public void doAction() throws LCException {
            ...
            // Select pasted (if they are keys)
            Set<GridPartKeyComponentI> toSelect = compToSelect.stream().filter(c -> c instanceof GridPartKeyComponentI).map(c -> (GridPartKeyComponentI) c).collect(Collectors.toSet());
            if (!toSelect.isEmpty()) {
                SelectionController.INSTANCE.selectKeyComponents(toSelect);
            }
        }
