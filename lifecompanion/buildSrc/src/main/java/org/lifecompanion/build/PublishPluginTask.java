...
public abstract class PublishPluginTask extends DefaultTask {
    private static final Logger LOGGER = Logging.getLogger(PublishPluginTask.class);

    @TaskAction
    void publishPluginUpdate() throws Exception {
        // Find jar task
        Jar jar = (Jar) this.getProject().getTasksByName("jar", false).iterator().next();
        File pluginPath = jar.getArchiveFile().get().getAsFile();
        ...
        LOGGER.lifecycle("publishLauncherUpdate : env = {}, appId = {}, version = {}, visibility = {} ", env, appId, version, visibility);
